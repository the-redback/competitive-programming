...
int dfs_2nd(int u, int k) {
    ...
    for (int i = 0; i < te[u].size(); i++) {
        if (!color[te[u][i]]) dfs_2nd(te[u][i], k);
    }
}
...
string scc(int n) {
    ...
    for (i = 0; i < arr.size(); i++) {
        if (!color[arr[i]]) dfs_2nd(arr[i], k + 1), k++;
    }
    ...
    for (i = 0; i < v.size(); i++) {
        ...
        if (id[k] != id[l]) ne[id[k]].pb(id[l]), out[id[k]]++, in[id[l]]++, flag = 1;
    }
    int cnt = 0;
    if (out[id[0]] == 0 && flag) return "NO";
    for (i = 1; i <= node; i++) {
        if (in[i] != out[i]) {
            cnt++;
        }
        if (cnt > 2 || abs(in[i] - out[i]) >= 2) return "NO";
    }
    flag = 1;
    k = id[0];
    mem(color, 0);
    ...
    for (i = 0; i <= n; i++) {
        if (wish[i] && !color[id[i]]) return "NO";
    }
