...
struct point {
    double x, y, xx, yy;
    point() { x = 0, y = 0, xx = 0, yy = 0; }
    point(double X, double Y, double XX, double YY) { x = X, y = Y, xx = XX, yy = YY; }
};
...
main() {
    ...
    while (tc--) {
        ...
        for (i = 0; i < 4 && fl; i++) {
            ...
            if ((one * two) < 0 && three * four < 0) fl = 0;
            if (one == 0) {
                if (pq.x >= min(v[i].x, v[i].xx) && pq.x <= max(v[i].x, v[i].xx) && pq.y >= min(v[i].y, v[i].yy) &&
                    pq.y <= max(v[i].y, v[i].yy))
                    fl = 0;
            } else if (two == 0) {
                if (pq.xx >= min(v[i].x, v[i].xx) && pq.xx <= max(v[i].x, v[i].xx) && pq.yy >= min(v[i].y, v[i].yy) &&
                    pq.yy <= max(v[i].y, v[i].yy))
                    fl = 0;
            } else if (three == 0) {
                if (v[i].x >= min(pq.x, pq.xx) && v[i].x <= max(pq.x, pq.xx) && v[i].y >= min(pq.y, pq.yy) &&
                    v[i].y <= max(pq.y, pq.yy))
                    fl = 0;
            } else if (four == 0) {
                if (v[i].xx >= min(pq.x, pq.xx) && v[i].xx <= max(pq.x, pq.xx) && v[i].yy >= min(pq.y, pq.yy) &&
                    v[i].yy <= max(pq.y, pq.yy))
                    fl = 0;
            }
        }
        if (fl) {
            if (pq.xx >= min(qq.x, qq.xx) && pq.xx <= max(qq.x, qq.xx) && pq.yy >= min(qq.y, qq.yy) &&
                pq.yy <= max(qq.y, qq.yy))
                fl = 0;
            if (pq.x >= min(qq.x, qq.xx) && pq.x <= max(qq.x, qq.xx) && pq.y >= min(qq.y, qq.yy) &&
                pq.y <= max(qq.y, qq.yy))
                fl = 0;
        }
        if (fl)
