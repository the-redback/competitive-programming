    ...
    while (tc--) {
        ...
        for (i = 0; i < n; i++) {
            for (j = i + 1; j < n; j++) {
                ...
                double four =
                    CP(MV(P(v[j].x, v[j].y), P(v[j].xx, v[j].yy)), MV(P(v[j].x, v[j].y), P(v[i].xx, v[i].yy)));
                if ((one * two) < 0 && three * four < 0) a[i] = 1, a[j] = 1;
                if (one == 0) {
                    if (v[j].x >= min(v[i].x, v[i].xx) && v[j].x <= max(v[i].x, v[i].xx) &&
                        v[j].y >= min(v[i].y, v[i].yy) && v[j].y <= max(v[i].y, v[i].yy))
                        a[i] = 1, a[j] = 1;
                }
                if (two == 0) {
                    if (v[j].xx >= min(v[i].x, v[i].xx) && v[j].xx <= max(v[i].x, v[i].xx) &&
                        v[j].yy >= min(v[i].y, v[i].yy) && v[j].yy <= max(v[i].y, v[i].yy))
                        a[i] = 1, a[j] = 1;
                }
                if (three == 0) {
                    if (v[i].x >= min(v[j].x, v[j].xx) && v[i].x <= max(v[j].x, v[j].xx) &&
                        v[i].y >= min(v[j].y, v[j].yy) && v[i].y <= max(v[j].y, v[j].yy))
                        a[i] = 1, a[j] = 1;
                }
                if (four == 0) {
                    if (v[i].xx >= min(v[j].x, v[j].xx) && v[i].xx <= max(v[j].x, v[j].xx) &&
                        v[i].yy >= min(v[j].y, v[j].yy) && v[i].yy <= max(v[j].y, v[j].yy))
                        a[i] = 1, a[j] = 1;
                }
            }
            if (a[i] == 0) res++;
