    ...
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        while(low <= high){
            ...
            if(cut1 < n1 && nums2[cut2 - 1] > nums1[cut1])
                ...
                if(cut1 == 0) maxOfLeft = nums2[cut2-1];                
                else if(cut2 == 0) maxOfLeft = nums1[cut1-1];
                else maxOfLeft = max(nums1[cut1 - 1], nums2[cut2 - 1]);

                if( (n1 + n2)%2 )
                    return maxOfLeft;

                int minOfRight=INT_MAX;
                if(cut1 == n1) minOfRight = nums2[cut2];                
                else if(cut2 == n2) minOfRight = nums1[cut1];
                else minOfRight = min(nums1[cut1], nums2[cut2]);
