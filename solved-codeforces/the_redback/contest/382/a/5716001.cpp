...
int main() {
    ...
    while (cin >> s) {
        ...
        if ((s.size() + ss.size()) % 2 == 0)
            ...
            for (i = 0; i < r; i++) ans.push_back(s[i]);
            for (i = 0; ans.size() != mid; i++) ans.push_back(ss[i]), k = i;
            ans.push_back('|');
            for (i = r + 1; i < s.size(); i++) ans.push_back(s[i]);
            for (i = k + 1; i < ss.size(); i++) ans.push_back(ss[i]);
            printf("%s\n", ans.c_str());
        }
    }
    return 0;
