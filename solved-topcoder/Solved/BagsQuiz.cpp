    ...
    int checkIfProper(int n, vector<string> act) {
        ...
        for (i = 0; i < act.size(); i++) {
            ...
            if (strcmp(aa, "PUT") == 0) {
                ...
                if (flag[k] != -1 || flag[l] != -1) return -1;
                flag[k] = l;
            } else if (strcmp(aa, "SET") == 0) {
                sscanf(act[i].c_str(), "%*s %d %*s", &k);
                if (flag[k] != -1) return -1;
                for (j = 1; j <= n; j++)
                    if (flag[j] == k) flag[j] = -1;
            } else {
                sscanf(act[i].c_str(), "%*s %d %*s %d", &k, &l);
                if (flag[k] != -1 || flag[l] != -1) return -1;
                for (j = 1; j <= n; j++)
                    if (flag[j] == k) flag[j] = inf;
                for (j = 1; j <= n; j++)
                    if (flag[j] == l) flag[j] = k;
                for (j = 1; j <= n; j++)
                    if (flag[j] == inf) flag[j] = l;
            }
        }
        ret = 0;
        for (j = 1; j <= n; j++) {
...
private:
    ...
    void test_case_0() {
        int Arg0 = 9;
        string Arr1[] = {"PUT 1 INSIDE 9", "SET 6 LOOSE",    "PUT 9 INSIDE 7", "SWAP 8 WITH 7",
                         "PUT 6 INSIDE 8", "PUT 3 INSIDE 7", "SET 2 LOOSE",    "PUT 4 INSIDE 7",
                         "PUT 8 INSIDE 2", "PUT 7 INSIDE 2", "PUT 5 INSIDE 2", "SET 2 LOOSE",
                         "PUT 2 INSIDE 5", "PUT 5 INSIDE 8", "PUT 7 INSIDE 8", "SET 8 LOOSE"};
